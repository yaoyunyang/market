...
Page({
  ...
  data: {
    ...
    interval: 5000,
    duration: 1000,
    imgUrls: [{ id: '1', path: 'cloud://market-f4n7q.6d61-market-f4n7q/ad/ad1.png' },
      { id: '2', path: 'cloud://market-f4n7q.6d61-market-f4n7q/ad/ad2.png' },
      { id: '3', path: 'cloud://market-f4n7q.6d61-market-f4n7q/ad/ad3.png' }],
  ...
  get_data_from_cloud: function () {
    ...
    //显示物品信息
    db.collection('goods').orderBy('creatTime', 'desc').limit(from_start).get({
      success(res) {
  ...
  get_more_data_from_cloud: function () {
    ...
    console.log('==调用加载物品函数==')
    //显示物品信息
    db.collection('goods').orderBy('creatTime', 'desc').limit(from_start).skip(from_start).get({
      success(res) {
        console.log('==加载物品成功==搜索页==')
        let local_data = res.data
        let local_attention = wx.getStorageSync('attention_cookies') || []
        ...
        that.data.products_homepage.push.apply(that.data.products_homepage, local_data)
        that.setData({
          products_homepage: that.data.products_homepage
        })
        wx.setStorageSync('goods_cookies', that.data.products_homepage)
        console.log('==更新后的数据==')
        console.log(that.data.products_homepage)
      }
    });
  ...
  //   console.log(res)
  //   wx.previewImage({
  //     urls: [that.data.imgUrls[res.currentTarget.dataset.id_].path]
  //   })
  // },
